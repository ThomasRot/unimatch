...
def inference_flow(
    ...
    frame_idx = 0
    for image1, image2 in pairs(filenames):
        print(f"Computing flow from Frame {frame_idx} -> Frame {frame_idx + 1}")
        frame_idx += 1
        test_id = frame_idx
        ...
        if inference_video is not None:
            output_file = os.path.join(output_path, "%04d_flow.npy" % test_id)
        else:
        ...
        if inference_video is not None and save_video:
            ...
            with open(output_file, "wb") as f:
                np.save(f, flow)
            save_vis_flow_tofile(flow, output_file.replace(".npy", ".png"))
